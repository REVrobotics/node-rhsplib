...
// @TODO fix function number. The function number probably wrong in 1.9.0 spec.
int RHSPlib_deviceControl_readVersion(RHSPlib_Module_T *obj, RHSPlib_Version_T *version, uint8_t *nackReasonCode)
{
    ...
    int retval = RHSPlib_getInterfacePacketID(obj, "DEKA", 48, &packetID, nackReasonCode);
    if (retval < 0)
    {
    	return retval;
    }
    ...
    if (version)
    {
        ...
        version->hwRevision          = RHSPLIB_ARRAY_BYTE(uint8_t, RHSPLIB_PACKET_PAYLOAD_PTR(obj->rxBuffer), 3);
        version->hwType              = RHSPLIB_ARRAY_DWORD(uint32_t, RHSPLIB_PACKET_PAYLOAD_PTR(obj->rxBuffer), 4);
    }
    return RHSPLIB_RESULT_OK;
}
                                             ...
                                             uint8_t *textLength, char *text, uint8_t *nackReasonCode)
{
    ...
    if (textLength && text)
    {
        ...
        retval = snprintf(fw_version_formatted, VERSION_FORMATTED_LENGTH, "%u.%u.%u", maj, min, eng);
